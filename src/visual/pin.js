...
class Pin {
  ...
  createConnection(pin) {
    ...
    //       this.connection[0].view.render();
    //       this.connection[0].connection = null;
    //     }
  ...
  init(node, direction, index) {
    ...
    this.connections = [];
    this.maxConnections = (this.direction === 'in' || this.type === 'flow') ? 1 : 20;
    this.index = index;
    this.svg = node.svg;
  ...
  getOffsets() {
    return {
      x: this.node.x + (this.direction === Pin.Direction.IN ? 10 : 100 - 20),
      y: this.node.y + 15 + (15 * this.index)
    };
  }
...
class PinValue extends Pin {
  ...
  compile() {
    ...
    return (this.direction === Pin.Direction.IN) ? this.connections[0].compile() : this.node.compile();
  }
}
